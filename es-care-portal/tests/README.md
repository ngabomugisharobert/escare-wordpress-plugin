...
- Job seeker: register, activate from the email link, apply, take an assessment, submit a service request. Confirm Apply is visible on Careers.
- Employer: register, activate from the email link, submit a job (stays pending), receive moderation email. Confirm Apply is hidden on Careers while signed in.
- Portal admin (portal session on `/sign-in/`, not WordPress `/wp-admin/`): manage users and jobs, paginate users/jobs/applications. Open Careers and a job page — Apply must not appear. Dashboard menu is Overview, Users, Jobs, Applications (no Contact Us).
