...
class ESC_Portal_Admin {
	...
	public static function init() {
		...
		add_action( 'admin_post_esc_create_portal_admin', array( __CLASS__, 'handle_create_portal_admin' ) );
	...
	public static function menu() {
		...
		add_submenu_page(
			'esc-portal',
			__( 'Portal Users', 'es-care-portal' ),
			__( 'Portal Users', 'es-care-portal' ),
			'manage_options',
			'esc-portal-users',
	...
	/**
	 * Create a portal admin from WP admin.
	 */
	public static function handle_create_portal_admin() {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'You are not allowed to do that.', 'es-care-portal' ) );
		}

		$nonce = isset( $_POST['esc_portal_admin_nonce'] ) ? sanitize_text_field( wp_unslash( $_POST['esc_portal_admin_nonce'] ) ) : '';

		if ( ! wp_verify_nonce( $nonce, 'esc_create_portal_admin' ) ) {
			wp_die( esc_html__( 'The form expired. Please try again.', 'es-care-portal' ) );
		}
	...
	public static function notices() {
		if ( empty( $_GET['esc_updated'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			return;
		}
